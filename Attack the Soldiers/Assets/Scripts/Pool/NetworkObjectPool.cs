...
public class NetworkObjectPool : NetworkBehaviour
{
    ...
    Dictionary<string, PoolConfigObject> m_configObjectByName = new Dictionary<string, PoolConfigObject>();
    ...
    public void InitializeObjectPool()
    {
        ...
        foreach (var configObject in PooledPrefabsList)
        {
            ...
            if (!m_configObjectByName.ContainsKey(configObject.Prefab.name))
                m_configObjectByName.Add(configObject.Prefab.name, configObject);
        }
    }
    ...
    /// <returns></returns>
    public NetworkObject GetNetworkObject(string prefabName, Vector3 position, Quaternion rotation)
    {
    ...
    {
        if (m_configObjectByName.TryGetValue(prefabName, out var configObject))
        {
            return configObject.Prefab;
        }
        return null;
    }
    ...
    public void ReturnNetworkObjectByName(NetworkObject networkObject, string prefabName)
    {
        m_PooledObjects[GetPrefabByName(prefabName)].Release(networkObject);
    }
...
class PooledPrefabInstanceHandler : INetworkPrefabInstanceHandler
{
    ...
    {
        return m_Pool.GetNetworkObject(m_Prefab.name, position, rotation);
    }
