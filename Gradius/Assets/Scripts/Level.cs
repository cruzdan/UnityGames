...
public class Level : MonoBehaviour
{
    ...
    void Start()
    {
        progress = Squares.totalSquaresX / 14f;
    ...
    {
        float position = Squares.totalSquaresX / 2f + newProgress * progress;
    ...
    void GeneratePhaseInfo(float newProgress, int phaseNumber)
    {
        ...
        info.transform.SetParent(parent);
        PhaseData pd = info.GetComponent<PhaseData>();
        pd.SetPhase(phaseNumber);
    }

    public void CheckPhaseData(Collider2D collision)
    {
        switch (collision.GetComponent<PhaseData>().GetPhase())
        {
            case 1:
                background.SetPause(true);
                phase = 1;
